<template>
  <div>
    <el-row :gutter="20">
      <el-col :span="12">
        <el-form ref="ruleForm" :model="form" :rules="rules" label-width="100px" :hide-required-asterisk="true">
          <el-form-item label="名称" prop="name">
            <el-input v-model="form.name" placeholder="请输入数据源名称" style="width: 220px"></el-input>
          </el-form-item>
          <el-form-item label="数据文件">
            <el-upload ref="fileUpload" class="upload" drag="drag" action="/" :on-change="importExcel" :on-exceed="onFileExceed" :on-remove="onFileRemove" :auto-upload="false" :limit="1">
              <i class="el-icon-upload"></i>
              <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
              <div slot="tip" class="el-upload__tip">请上传 Excel 表格文件，其中第一行为表头</div>
            </el-upload>
          </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        data: [{ type: 'array', required: true, min: 1, message: '请上传数据', trigger: 'change' }],
        dimension: [{ required: true, message: '请选择维度', trigger: 'change' }],
        metrics: [{ type: 'array', required: true, min: 1, message: '请选择指标', trigger: 'change' }]
      }
    }
  },
  computed: {
